...
|--------|----------|---------|
| `ka-` | I / my | `ka paai` → I go |
| `na-` | you / your | `na paai` → you go |
| `a-` | he / she / his / her | `a paai` → he goes |
| `i-` | we / our | `i paai` → we go |
...
|----------|---------|---------|
| `-te` | plural (like English -s) | `mi` + `te` → `mite` (people) |
| `-ding` | future / will | `paai` + `ding` → `paaiding` (will go) |
| `-ta` | perfective / already | `om` + `ta` → `omta` (exists already) |
| `-ah` | locative / in/at/on | `khawm` + `ah` → `khawmah` (in the gathering) |
| `-in` | instrumental / by/with | `khut` + `in` → `khutin` (by hand) |
| `-'n` | agentive (short for `in`) | `Zeisu` + `'n` → `Zeisu'n` (Jesus - subject) |
...
|-----------|------|----------|
| **Prefixes go RIGHT** 🔜 | `ki-` | `ki` + `cing` → `kicing` |
| | `ka-` | `ka` + `paai` → `ka paai` |
| **Suffixes go LEFT** 🔙 | `-te` | `mi` + `te` → `mite` |
| | `-ding` | `paai` + `ding` → `paaiding` |
| | `-ta` | `om` + `ta` → `omta` |
| | `-ah` | `khawm` + `ah` → `khawmah` |
| | `-'n` | `Zeisu` + `'n` → `Zeisu'n` |
