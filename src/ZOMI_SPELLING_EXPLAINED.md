...
**Zomi is spoken as single words, but often written as separate syllables.**
...
- `paai ding in` → `paaidingin` ("will go by")
- `mi te 'n` → `miten` ("people (agentive)")
...
|-----------|----------|---------|
| **Merge RIGHT** 🔜 | `ki` | `ki` + `sim` → `kisim` |
| | `i` / `ka` / `na` / `a` | `ka` + `paai` → `ka paai` (keep separate) |
| **Merge LEFT** 🔙 | `te` | `mi` + `te` → `mite` |
